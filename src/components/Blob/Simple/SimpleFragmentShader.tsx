...
vec3 blendedColor = u_color * u_colorIntensity;


// [2] 탁함 보정
blendedColor = mix(blendedColor, vec3(1.0), 0.35);
blendedColor = pow(blendedColor, vec3(1.0 / 2.2)); // 감마 보정

// [3] 수직 위치 기반 그라데이션
float gradientFactor = clamp((vPosition.y + 2.0) / 4.0, 0.0, 1.0); // -2~2 → 0~1
vec3 gradientColor = mix(u_color, vec3(1.0), gradientFactor);

// [4] blended + gradient 결합
vec3 Color = mix(blendedColor, gradientColor, 0.5);

// [5] 파면 색상 변동
float colorNoise = sin(vPosition.x * 3.0 + u_time) * cos(vPosition.y * 2.0 + u_time * 0.7);
Color = mix(Color, vec3(1.0), 0.1 + 0.1 * colorNoise);

// [6] 물 색상과 블렌딩
vec3 waterColor = vec3(0.0, 0.05, 0.2);
vec3 deepColor = vec3(0.0, 0.0, 0.3);
vec3 lightWaterColor = mix(waterColor, Color, 0.6);
vec3 deepWaterColor = mix(deepColor, Color * 0.8, 0.5);
...
vec3 rippleColor = mix(vec3(0.9, 0.95, 1.0), Color, 0.3) * abs(vDisplacement) * 0.3;
