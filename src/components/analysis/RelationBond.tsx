...
const RelationBond = () => {
  ...
  const onClickHandler = () => {
    navigator("/analysis/relationbond");
  };
  const { data, isLoading } = useMentalData("유대", 365);
  return (
    <div className="w-full mt-5 ">
      <div className="rounded-3xl shadow-xl bg-white">
        <div className="flex justify-between pt-3 ml-6 mr-5 mb-1">
          <h1 className="text-lg font-bold text-gray-900">유대</h1>
        ...
        <div className="h-44 content-end">
          <div className="h-fit">
            {isLoading ? <LoadingAnimation /> : <MentalChart type={"유대"} data={data.date} limit={4} />}
          </div>
        </div>
      </div>
    </div>
