...
const BottomNavi = ({ onMicClick, onLocationClick, isListening, inputFocused}: BottomNaviProps) => {


    return (
        <div className={`relative bottom-4 w-full h-[84px] mx-auto z-50 flex justify-center transition-transform duration-300 ease-in-out${
            inputFocused ? "translate-y-[-200px]" : "translate-y-0"
          }`}
        >
            {/* ✅ SVG는 절대 그대로 유지 */}
            <svg
            ...
            >
                ...
                <defs>
                    ...
                    <feBlend mode="normal" in="SourceGraphic" in2="effect1_dropShadow_548_3742" result="shape"/>
                    </filter>
                </defs>
            </svg>
            ...
            className={`w-[56px] h-[56px] rounded-full transition-all ${
                isListening ? 'bg-red-500 bg-opacity-20' : ''
            }`}
            onClick={onMicClick}
            />
            ...
            className="w-[52px] h-[52px] rounded-full cursor-pointer flex items-center justify-center 
            hover:bg-gray-600 hover:bg-opacity-20 transition-all"
            />
            
            {/* 위치 아이콘 버튼 */}
            <button 
            className="w-[52px] h-[52px] rounded-full hover:bg-blue-200 hover:bg-opacity-20 transition-all"
