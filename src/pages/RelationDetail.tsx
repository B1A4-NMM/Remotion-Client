...
const RelationDetail = () => {
    ...
    const analyzeRelation = (relationData: RelationData) => {
        ...
        const emotionCounts = allEmotions.reduce((acc: Record<string, {count: number, totalIntensity: number}>, emotion) => {
        ...
        const emotionStats = Object.entries(emotionCounts).map(([emotion, stats]) => ({
            emotion,
            count: stats.count,
        ...
        // 5. 최근 활동
        const recentDiary = diaries.sort((a, b) => 
    ...
    const calculateIntimacyScore = (relationData: RelationData): number => {
        ...
        const allEmotions = emotions.flatMap(day => day.emotions);
        const positiveRatio = allEmotions.filter(e => positiveEmotions.includes(e.emotion)).length / allEmotions.length;
        ...
        const recentMeeting = diaries.some(d => 
            getDaysBetween(new Date(d.writtenDate), new Date()) <= 30
        );
            ...
            <div className="max-w-4xl mx-auto px-4 space-y-8">
                ...
                >
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                        ...
                        <div className="bg-gray-50 rounded-xl p-4">
                            <div className="text-2xl font-bold text-gray-900">{analysis.topActivities.length}</div>
                            <div className="text-sm text-gray-600">공유한 활동</div>
                        </div>
                        <div className="bg-gray-50 rounded-xl p-4">
                            <div className="text-2xl font-bold text-gray-900">{Math.round(analysis.meetingFrequency)}</div>
                ...
                >
                    ...
                    <div className="space-y-3">
                        {analysis.topActivities.map(([activity, count], index) => (
                            <div key={activity} className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                ...
                >
                    ...
                    <div className="space-y-4 text-lg leading-relaxed text-black">
                        <p>
                            {userName}님은 <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.targetName}</span>과 지금까지 <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.totalDiaries}</span>번의 소중한 순간을 함께했습니다.
                        </p>
                        <p>
                            가장 많이 함께한 활동은 <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.topActivities[0]?.[0]}</span>이며, 
                            주로 <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.emotionStats[0]?.emotion}</span> 감정을 많이 나누었네요.
                        </p>
                        <p>
                            친밀도 점수 <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.intimacyScore}</span>점으로 보아, <span className="font-bold underline underline-offset-4"style={{ textDecorationColor: strongestEmotionColor}}>{analysis.intimacyScore >= 80 ? "매우 가까운" : 
                             analysis.intimacyScore >= 60 ? "친밀한" : 
                             analysis.intimacyScore >= 40 ? "좋은" : "발전하는"}</span> 관계를 유지하고 있습니다!
                        </p>
                    </div>
                </motion.div>
            </div>
