...
const Positive=()=>{
   const [selectedPeriod, setSelectedPeriod] = useState<PeriodType>("daily");
       ...
       const queries = groupMap["긍정"].map((emotion: MentalType) => useMentalData(emotion, period));
       ...
       return(
        ...
        <div className="px-4 py-8 text-foreground min-h-screen space-y-10">
            ...
            <section className="bg-white rounded-xl shadow p-6">
                <div className="text-xl font-bold p-3"> {getPeriodConfig(selectedPeriod).label}별 긍정적 감정 수치</div>
                    {isLoading ? (
                        <LoadingAnimation />
                    ) : (
                        <MentalChart type="긍정" data={data} limit={barCount}/>
                    )}
            </section>
            <section className="bg-white rounded-3xl shadow-xl mb-4 pb-10">
                ...
                <hr className="ml-4 mr-4 mb-6"/>
                    {isLoading?(
                        ...
                        <ActivitySection type="긍정" data={allActivityData} selectedPeriod={selectedPeriod}/>
                    )}
            </section>
            <section className="bg-white rounded-3xl shadow-xl mb-4">
                <div className="text-xl font-bold p-3 pb-5 pl-5 mt-10"> 긍정적 감정에 가장 큰 영향을 준 사람 TOP 3</div>
                <hr className="ml-4 mr-4 "/>
                    <PeopleSection type="긍정" data={allPeopleData} selectedPeriod={selectedPeriod}/>
            </section>
        </div>
    </div>
    )
